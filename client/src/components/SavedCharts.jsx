...
const SavedCharts = forwardRef(
  ({ uploadedFileId, rows, onChartClick }, ref) => {
    ...
    const filteredCharts = uploadedFileId
      ? charts.filter((chart) => chart.uploadedFile === uploadedFileId)
      : charts;
    ...
    return (
      <div className="flex flex-wrap gap-4 justify-center">
        {filteredCharts.map((chart, index) => (
          ...
          >
            <ChartCard chart={chart} index={index} rows={rows || []} readOnly />
          </div>
        ))}
      </div>
    );
